...
public class Player extends CharacterEntity {
    ...
    /**
     * This method calculates the players attackValue.
     * It searches for equipped weapons, and adds them to the base attackValue
    ...
    public int getAttackValue() {
        ...
        for (Item item : equipableInventory.getInventory()) { //Goes through our equipableInventory, to see if we have a weapon equipped
            if (item.getItemType().equals(ItemType.WEAPON)) {
                attackValue += item.getItemValue(); //If we have a weapon equipped, it gets added to our attackValue
                break;
            }
        }
        return attackValue;
    ...
    /**
     * This method calculates the players armorValue
     * It searches for equipped armor, and adds it to the base armorValue
    ...
    public int getArmorValue() {
        ...
        for (Item item : equipableInventory.getInventory()) { //Goes through our equipableInventory and checks if we have any armor equipped
            if (item.getItemType().equals(ItemType.ARMOR)) {
                armorValue += item.getItemValue(); //If we have any armor equipped, it gets added to our armorValue
                break;
            }
        }
        return armorValue;
    ...
    /**
     * This method is used when the player equips either a weapon or a piece of armor
     * @param item It takes an item as a parameter, and that item gets equipped into equipableInventory, if it's a weapon or piece of armor
     * @return If the item was a weapon or a piece of armor, and it was equipped, it returns true. If item did not get equipped, returns false
     */
    public boolean equip(Item item) {
        if (itemInventory.removeItem(item, 1)) { //Removes the item that we want to equip from our inventory
            if (item.getItemType().equals(ItemType.WEAPON)) { //Checks if the item is a weapon
                for (Item i : equipableInventory.getInventory()) { //Searches through our equipableInventory
                    if (i.getItemType().equals(ItemType.WEAPON)) {  //If we find a weapon that is currently equipped, it does:
                ...
                return true;
            } else if (item.getItemType().equals(ItemType.ARMOR)) { //Checks if the item is a piece of armor
                for (Item i : equipableInventory.getInventory()) { //Searches through our equipableInventory
                    if (i.getItemType().equals(ItemType.ARMOR)) {   //If we find a piece of armor that is currently equipped, then do:
     ...
     * @return Returns true if health restoring was successful, otherwise returns false
     */
    public boolean restoreHp(Item item) {
        if (!(this.health >= this.maxHealth)) { //If player is already at max health, returns false
            if (item.getItemType().equals(ItemType.CONSUMEABLE)) { //Checks if the item is a consumeable (only type of item that can restore health)
                if (this.itemInventory.removeItem(item, 1)) { //Removes the item from our inventory
                    if (this.health + item.getItemValue() >= this.maxHealth) { //If the health restored + our current health is greater than our max health, 
                        // sets our health = max health. This is to prevent gaining more health.
                        this.health = this.maxHealth;
                        return true;
                    } else {
                        this.health += item.getItemValue(); //Adds the health to the player
                        return true;
                    }
                }
            }
     ...
     * @param room It takes a room as parameter, to check if the required quest giver is in the room
